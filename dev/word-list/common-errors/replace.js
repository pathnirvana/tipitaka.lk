...
fs.readFileSync(path.join(__dirname, checkedFilename), 'utf-8').split('\n').forEach((line, lineNum) => {
    const cells = line.split('\t').filter(c => c.trim()).map(cell => cell.match(/([\u0D80-\u0DFF\u200d!]+)\/(\d+)\/\d+([mcdx]?)/)) // for pali need extra \/\d+
    //console.log(cells)
...
const perf1 = perf.now(), fileFilter = file => !/^(atta|anya)/.test(file) ///^(vp-cv|kn-mn|kn-nc)/.test(file)    //
const modCounts = processTextFiles(fileFilter, (data, file) => makeReplacements(data), dryRun)
